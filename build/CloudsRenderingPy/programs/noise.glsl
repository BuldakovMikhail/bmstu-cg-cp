...
vec4 stackable3DNoise(vec3 pixel)
{
	...
	vec3 coord = vec3(float(pixel.x) * ar / float(u_resolution.x), float(pixel.y) / float(u_resolution.y), float(pixel.z) / 128.0);

	// Perlin FBM noise
	int octaveCount = 3;
	float frequency = 8.0;
	float perlinNoise = max(perlinNoise3D(coord, frequency, octaveCount), 0.6);
